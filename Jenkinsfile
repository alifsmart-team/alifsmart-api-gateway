        ...
        stage('Build & Push Docker Image') {
            steps {
                script {
                    ...
                    // Definisikan nama image dasar tanpa tag registry di awal jika menggunakan docker.withRegistry
                    // Plugin akan menangani prefix registry secara otomatis.
                    def imageName = "${env.DOCKER_HUB_USERNAME}/${env.DOCKER_IMAGE_NAME}"
                    def buildTag = env.BUILD_NUMBER // Tag unik berdasarkan nomor build Jenkins
                    def latestTag = "latest"

                    // Menggunakan docker.withRegistry untuk login yang aman dan terintegrasi
                    // Pastikan DOCKER_HUB_CREDENTIALS_ID adalah ID kredensial "Username with password"
                    // untuk Docker Hub yang sudah Anda simpan di Jenkins.
                    docker.withRegistry("https://index.docker.io/v1/", env.DOCKER_HUB_CREDENTIALS_ID) {
                        
                        // 1. Build image dan beri tag dengan BUILD_NUMBER
                        ...
                        // 2. Beri tag 'latest' pada image yang sama
